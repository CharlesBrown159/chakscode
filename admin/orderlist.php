...
<body>
    ...
    <div class="container" style="padding-top: 150px">
        ...
        while ($row = array_shift($result)) {
            ...
            $OPuname = $row['op_username'];

            $sqlOP = "SELECT * FROM `useraccount` WHERE ua_username = '$OPuname'";
            $pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
            $stmtOP = $pdo->prepare($sqlOP);
            $stmtOP->execute();
            $resultOP = $stmtOP->fetchAll();

            while ($rowOP = array_shift($resultOP)) {
                $fname_OP = $rowOP['ua_fullname'];
            }
            $op_id = $row['op_id'];
        ...
        ?>
            ...
            <div class="card shadow p-3 mb-5 rounded">
                ...
                <div class="row my-4">
                    <div class="col-sm">
                        <h3>Waiter: <?php echo $fname_OP; ?></h3>
                    </div>
                    <div class="col-sm">
                    ...
                    <div class="col-sm">
                        <h3>Total Price: <span class="text-danger">₱<?php echo $row['op_price']; ?></span></h3>
                    </div>
                </div>
